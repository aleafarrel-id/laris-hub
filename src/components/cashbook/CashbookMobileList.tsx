...
interface CashbookMobileListProps {
 ...
 */
export function CashbookMobileList({
  transactions,
  isAdmin,
  isLoading,
  onEditTransaction,
        ...
        {transactions.map((tx, idx) => (
          <motion.div
            ...
            className={`relative app-card p-4 flex items-stretch gap-3 ${
              tx.type === 'sale' && tx.status === 'pending' ? 'bg-amber-50/30 border-amber-200' : ''
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                tx.type === 'sale' ? 'bg-success/10' : 'bg-danger/10'
              }`}
            >
              {tx.type === 'sale' ? (
