...
import {
  useCashierAuthDetails,
  useDeleteCashier,
  useUpdateCashier,
} from '@/hooks/useCashierManagement'
    ...
    <Portal className="z-[60] flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="text-sm text-neutral-600 leading-relaxed mb-1">
          Akun <strong className="text-neutral-900">{cashier.full_name}</strong> akan dihapus
          permanen dari sistem.
        </p>
        <p className="text-sm text-neutral-500 mb-5">
          Jika cashier memiliki data transaksi, penghapusan akan ditolak secara otomatis, gunakan{' '}
...
export function CashierDetailDrawer({
  isOpen,
  cashier,
  onClose,
  onToggle,
  ...
  const [cachedCashier, setCachedKasir] = useState<Profile | null>(cashier)

  useEffect(() => {
    if (cashier) setCachedKasir(cashier)
  }, [cashier])

  const cashierToRender = cashier || cachedCashier

  const [isEditing, setIsEditing] = useState(false)
  const [showNewPassword, setShowNewPassword] = useState(false)
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false)
  const [editForm, setEditForm] = useState({
    full_name: cashierToRender?.full_name || '',
    phone: cashierToRender?.phone ?? '',
    email: '',
    password: '',
  })
  ...
  const { data: authDetails, isLoading: isLoadingEmail } = useCashierAuthDetails(
    cashierToRender?.id || '',
  )
  ...
  const { mutate: deleteCashier, isPending: isDeleting } = useDeleteCashier()
  ...
  const handleSave = () => {
    if (!validateEdit() || !cashierToRender) return
    updateCashier({
      id: cashierToRender.id,
  ...
  const isOwnAccount = cashierToRender?.id === adminId

  if (!cashierToRender) return null
  ...
  return (
    <>
        ...
        {isOpen && (
          <Portal className="z-50 flex">
            ...
            >
              ...
              {/* Profile hero */}
              <div
                className={`px-5 py-5 border-b border-neutral-100 ${cashierToRender.is_active ? 'bg-neutral-50/30' : 'bg-amber-50/40'}`}
              >
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <CashierAvatar profile={cashierToRender} size="lg" />
                    <span
                      className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${cashierToRender.is_active ? 'bg-emerald-500' : 'bg-amber-500'}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-lg font-bold text-neutral-900 leading-tight truncate">
                      {cashierToRender.full_name}
                    </p>
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full mt-2 ${cashierToRender.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                    >
                      ...
                      {cashierToRender.is_active ? 'Aktif' : 'Ditangguhkan'}
                    </span>
                  </div>
                </div>
              </div>
                ...
                {!isEditing ? (
                  <>
                    <div className="mb-5">
                      ...
                      />
                      <InfoRow icon={Phone} label="No. HP" value={cashierToRender.phone} />
                      <InfoRow
                        icon={Calendar}
                        label="Bergabung Sejak"
                        value={formatDate(cashierToRender.created_at as string)}
                      />
                      <InfoRow
                        icon={Clock}
                        ...
                        value={
                          authDetails?.last_sign_in_at
                            ? formatDateTime(authDetails.last_sign_in_at as string)
                            : 'Belum pernah login'
                        }
                        isLoading={isLoadingEmail}
                      />
                    ...
                    <div className="space-y-2.5">
                      <Button
                        ...
                        className="w-full"
                        disabled={cashierToRender.isOfflinePending}
                      ...
                      {!isOwnAccount && (
                        ...
                        >
                          {cashierToRender.is_active ? 'Tangguhkan Akun' : 'Aktifkan Kembali'}
                        </Button>
                      )}
      ...
      <AnimatePresence>
        {showDeleteConfirm && (
          <DeleteConfirmModal
            cashier={cashierToRender}
            onConfirm={() =>
              deleteCashier(cashierToRender.id, {
                onSuccess: () => {
                  setShowDeleteConfirm(false)
                  onClose()
                },
