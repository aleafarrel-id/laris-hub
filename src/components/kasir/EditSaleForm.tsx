...
import { useUpdateSale } from '@/hooks/useTransactions'
...
export function EditSaleForm({
  ...
  useEffect(() => {
    if (!productsLoading && products.length > 0 && transaction.transaction_items) {
      const initialCart = new Map<string, CartItem>()
      transaction.transaction_items.forEach((item) => {
        const product = products.find((p) => p.id === item.product_id)
        // If product was deleted but still in transaction, we create a fallback product representation
        ...
        const key = item.product_id || crypto.randomUUID()
        initialCart.set(key, {
          product: productData,
  ...
  }, [transaction, products, productsLoading])
  ...
  }, [cart, cartArray, notes, updateSale, onSuccess, transaction])

  // Include missing legacy products that are in the cart but not in the products list
