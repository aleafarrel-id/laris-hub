...
export function StatusBadge({ status, forceShow }: { status: string | null; forceShow?: boolean }) {
  ...
  const config = {
    ...
    label: status,
    className: 'bg-neutral-50 text-neutral-600 border border-neutral-200',
  }
...
export function PaymentMethodBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold ${config.className}`}
    >
      {Icon && <Icon size={12} />}
      {config.label}
    </span>
